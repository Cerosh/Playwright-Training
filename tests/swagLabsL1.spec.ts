...
test.describe("Select items for checkout, adjust as needed, and ensure user information is complete before checkout.", () => {
  test("Check out a random number of items", async ({ page }) => {
    ...
    const numberOfItemsToAdd = Math.floor(
      Math.random() * (maxNumberOfItems - minNumberOfItems + 1) +
        minNumberOfItems,
    );
    ...
    await page
      .locator("a")
      .filter({ hasText: `${numberOfItemsToAdd}` })
      .click();
    await expect(
      page,
  ...
  test("Add more items during checkout.", async ({ page }) => {
    ...
    await page
      .locator("a")
      .filter({ hasText: `${totalNumberOfItems}` })
      .click();
    for (const item of arrayOfItems) {
      const itemName = item.itemName;
      const itemPrice = item.itemPrice;
